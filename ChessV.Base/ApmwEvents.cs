...
namespace ChessV.Base
{
  public class Starter
  {
    ...
    /** Possibly not stable - will generate a different pocket distribution as the player progresses through different foundPockets - but it is close to uniform */
    public (int, int, int) generatePocketValues(int foundPockets)
    {
      ...
      var y = random.Next(Math.Max(0, foundPockets - 8), Math.Min(foundPockets, 5));
      if (x < y)
      {
        (x, y) = (y, x);
      }
