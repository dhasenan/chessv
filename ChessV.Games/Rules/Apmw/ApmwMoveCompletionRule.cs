...
{
	public class ApmwMoveCompletionRule: MoveCompletionRule
	{
		// TODO(chesslogic): get the number of starting extra moves from apmw client

		// *** CONSTRUCTION *** //

		public ApmwMoveCompletionRule()
		{
			currentState = 1;
		}
		...
		{
			Starter.getInstance().MoveCompletionHandler.ForEach((handler) => handler(move));
			turnNumber++;
			Game.CurrentSide = Game.CurrentSide + 1 / 2;
		}
		...
		public override void UndoingMove()
    {
      ...
      Game.CurrentSide = Game.CurrentSide - 1 / 2;
		...
		protected string[] stateNotations = new string[] { "w", "b" };
	}
}
